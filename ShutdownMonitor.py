...
class Monitor:
    DbusService: VeDbusService = None
    DbusBus = None

    # read the GPIO pin and detect transition to active
    # only the TRANSITION causes action

    def detect_pin(self):
        if self.ShutdownPinPresent:
            gpio_file = "/sys/class/gpio/gpio16/value"
            if os.path.isfile(gpio_file):
                file = open(gpio_file, "r")
    ...
    def __init__(self):
        ...
        platform_file = "/etc/venus/machine"
        self.ShutdownPinPresent = False
        try:
            file = open(platform_file, "r")
        except Exception:
            self.ShutdownPinPresent = False
        else:
    ...
    def _create_dbus_service(self):
        ...
        self.DbusSettings = SettingsDevice(bus=dbus.SystemBus(), supportedSettings=settings_list, timeout=10, eventCallback=None)
